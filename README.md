...
- `--no-factions` prevents the Singularity faction manager from launching. When faction management is enabled, it adds **Tian Di Hui** to every BitNode faction route, reserves the cash needed to travel to Chongqing and join it, then buys its non-NeuroFlux augmentations before the rest of the route. It then joins the profile's other invited factions, works **Hacking Contracts** for missing augmentation reputation, buys relevant non-NeuroFlux augmentations from most expensive to least expensive, and starts its exit sequence after 10 queued augmentations: one final **NeuroFlux Governor** pass followed by installation and restart through `/bootstrap.js`. Once no non-NeuroFlux augmentations remain before that threshold, it spends available money (respecting the configured buffer) on as many **NeuroFlux Governor** levels as it can buy. When Daedalus is available and donation is unlocked, it donates only the reputation needed for each next NeuroFlux level and buys it before continuing. Use `--min-augmentations`, `--augmentation-money-buffer`, or `--no-install` to adjust that reset policy.

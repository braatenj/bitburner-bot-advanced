...
- `--no-factions` prevents the Singularity faction manager from launching. When faction management is enabled, it adds **Tian Di Hui** to every BitNode faction route, reserves the cash needed to travel to Chongqing and join it, then buys its non-NeuroFlux augmentations before the rest of the route. It then joins the profile's other invited factions, works **Hacking Contracts** for missing augmentation reputation, buys relevant non-NeuroFlux augmentations from most expensive to least expensive, and installs/restarts through `/bootstrap.js` after three queued augmentations. Once no non-NeuroFlux augmentations remain, it spends available money (respecting the configured buffer) on as many **NeuroFlux Governor** levels as it can buy. With `Formulas.exe` and enough current faction favor, it stops faction work and donates exactly enough money to reach the highest remaining augmentation reputation requirement. Use `--min-augmentations`, `--augmentation-money-buffer`, or `--no-install` to adjust that reset policy.
